...
const uint16_t kBtnLearn = 12;  // D6
const uint16_t kBtnBlast = 13;  // D7
const uint16_t kStatusLed = 5;  // D1 (Status LED)

const uint16_t kCaptureBufferSize = 200;

IRsend irsend(kIrLedPin);
IRrecv irrecv(kRecvPin, kCaptureBufferSize, 15, true); 
decode_results results;

uint16_t rawArray[kCaptureBufferSize];
...
void setup() {
  ...
  pinMode(kBtnLearn, INPUT_PULLUP); 
  pinMode(kBtnBlast, INPUT_PULLUP); 
  pinMode(kStatusLed, OUTPUT);
  
  digitalWrite(kStatusLed, LOW);
  Serial.println("--- SYSTEM START: STANDBY ---");
}

void loop() {
  // --- BUTTON 1: LEARN ---
  if (digitalRead(kBtnLearn) == LOW && !isLearning) {
    delay(50); 
    if (digitalRead(kBtnLearn) == LOW) {
      digitalWrite(kStatusLed, HIGH); // LED ON when clicked
      isLearning = true;
      Serial.println(">>> LEARN MODE: Point remote and press...");
      irrecv.resume();
      
      while(digitalRead(kBtnLearn) == LOW) { yield(); }
      delay(50); 
    }
  }
  ...
  // --- LED BLINKING LOGIC ---
  if (isLearning) {
    // Fast blink while learning (every 100ms)
    ...
    if (irrecv.decode(&results)) {
      savedRawLength = results.rawlen - 1; 
      for (uint16_t i = 1; i < results.rawlen; i++) {
        rawArray[i - 1] = results.rawbuf[i] * kRawTick;
      }
      
      // SUCCESS! Quick flash pattern
      for(int i=0; i<3; i++) {
        digitalWrite(kStatusLed, HIGH); delay(50);
        digitalWrite(kStatusLed, LOW); delay(50);
      }
      
      Serial.println("SUCCESS! Captured. Returning to Standby.");
      isLearning = false;
      irrecv.resume();
    }
  }
  ...
  // --- BUTTON 2: BLAST ---
  if (digitalRead(kBtnBlast) == LOW && !isLearning) {
    delay(50);
    if (digitalRead(kBtnBlast) == LOW) {
      digitalWrite(kStatusLed, HIGH); // LED ON during blast
      
      if (savedRawLength > 0) {
        Serial.println("BLASTING SAVED CODE...");
        irrecv.pause();
        irsend.sendRaw(rawArray, savedRawLength, 38); 
        delay(200);
        irrecv.resume();
        Serial.println("DONE.");
      } else {
        Serial.println("ERROR: No code saved!");
        // Error double flash
        digitalWrite(kStatusLed, LOW); delay(200);
        digitalWrite(kStatusLed, HIGH); delay(200);
      }
      
      while(digitalRead(kBtnBlast) == LOW) { yield(); }
      digitalWrite(kStatusLed, LOW);
      delay(50);
    }
  }
...
void loop() {
    ...
    handleUI(now);
>>>>>>> d7ebdcf (Initial commit: ESP8266 IoT Remote Buzzer Alarm)
}